...
Shim 可以信任一个公钥证书（在信任公钥证书时 Shim 会向 efivar 写入 MOK（Machine Owner Key）），这样所有被这个私钥证书签名过的引导程序/内核也就获得了信任，并可以安全启动它们了。这样做的好处是每次更新引导程序/内核时，你只需要重新签名引导程序/内核即可。无需再向 efivar 写入些什么。你可以通过包管理器的钩子实现在每次更新引导程序/内核时自动签名它们。

{{< callout type="tip" title="推荐方案" >}}
